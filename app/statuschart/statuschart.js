            ...
            $http({
                ...
                .then(function (response) {
                    ...
                    for (var data in $scope.hourwisemachine.run_time) {
                        var ss1 = $scope.hourwisemachine.run_time[data]/1 ;
                        var ss = Math.floor(ss1 / 60);
                    ...
                    for (var data in $scope.hourwisemachine.idle_time) {
                        var ss1 = $scope.hourwisemachine.idle_time[data] /1;
                        var ss = Math.floor(ss1 / 60);
                    ...
                    for (var data in $scope.hourwisemachine.stop_time) {
                        var ss1 = $scope.hourwisemachine.stop_time[data] / 1;
                        var ss = Math.floor(ss1 / 60);
                        $scope.stoparry.push(ss);
                    }

                    $scope.noarry = [];
                    for (var data in $scope.hourwisemachine.no_data) {
                        var ss1 = $scope.hourwisemachine.no_data[data] /1 ;
                        var ss = Math.floor(ss1 / 60);
                        $scope.noarry.push(ss);
                    }
                        ...
                        plotOptions: {
                            series: {
                                ...
                                dataLabels: {
                                    enabled: true,
                                    color: (Highcharts.theme && Highcharts.theme.dataLabelsColor) || 'white',
                                    format: '{y}m', // your label's value plus the percentage sign
                                    valueDecimals: 2 
                                }
                            }
